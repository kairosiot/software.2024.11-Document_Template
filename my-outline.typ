#let my-outline-row( textSize:none,
  ...
  box(width: 1.2cm, inset: (y: insetSize), align(left, number))
  h(0.1cm)
  box(inset: (y: insetSize), width: 100% - 1.3cm, )[
    #link(location, title)
...
#let my-outline(appendix_state, part_state, part_location,part_change,part_counter, mainColor, textSize1:none, textSize2:none, textSize3:none, textSize4:none, img:none) = {
  show outline: set heading(numbering: none, level: 2)
  show outline.entry: it => {
    ...
    if it.level == 1 {
      ...
      if (part_change){
        v(0.7cm, weak: true)
        box(width: 1.1cm, fill: mainColor.lighten(80%), inset: 5pt, align(center, text(size: textSize1, weight: "bold", fill: mainColor.lighten(30%), numbering("1",part_counter.first()))))
        h(0.1cm)
...
#let my-outline-small(partTitle, appendix_state, part_state, part_location,part_change,part_counter, mainColor, textSize1:none, textSize2:none, textSize3:none, textSize4:none, textSize5:none) = {
  show outline.entry: it => {
    let appendix_state = appendix_state.at(it.element.location())
    let numberingFormat = if appendix_state != none {"A.1"} else {"1.1"}
    let counterInt = counter(heading).at(it.element.location())
    ...
    if (part_state == partTitle and counterInt.first() >=0 and appendix_state==none){
      ...
      if it.level == 2 {
        ...
        my-outline-row(textWeight: "regular", textSize: textSize5, textColor:black, number: number, title: text(fill: black, title), heading_page: text(fill: black, heading_page), location: it.element.location())
      }
  ...
  box(width: 7.5cm, outline(depth: 4, indent: false, title: none))
}

#let my-outline-sec(listOfFigureTitle, target, textSize, img:none) = {
  show outline: set heading(numbering: none, level: 2)
